...
export function saveCachedSkills(
  ...
  const txn = db.transaction(() => {
    ...
    // scan.customPaths but those skills are not in the list". Custom scan paths yield
    // both "custom" and "project" scope rows, so both are kept.
    if (opts.preserveCustomScope) {
      db.prepare("DELETE FROM cached_skills WHERE scope NOT IN ('custom', 'project')").run()
    } else {
